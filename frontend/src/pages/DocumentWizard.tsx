...
import { DOCUMENT_TEMPLATES, DOCUMENT_CATEGORIES } from "@/data/documents";
...
const DocumentWizard = () => {
  ...
  const handleDownload = async () => {
    toast({ title: t("generatingDoc"), description: t("processing") });
    try {
      const res = await fetch("/api/generate-draft-pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ query: caseDesc, doc_type: selected?.category || "RTI", lang: docLang }),
      });
      ...
      a.href = url;
      a.download = `Draft_${selected?.name || "Document"}.pdf`;
